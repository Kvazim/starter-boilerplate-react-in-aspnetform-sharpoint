...
Для создания мастер файла сначала запускаем команду npm run server водим в косоли свои учетные данные, после запуска сервера запускаем npm run dev. после верстки и сборки проекта (npm run build) копируем скрипты в sharpoint и прописываем пути к ним в мастер файле react.master в ядре.
